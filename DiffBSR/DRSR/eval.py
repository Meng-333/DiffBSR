...
def eval():
    ...
    for batch in eval_data_loader:
        ...
        mse_pre = calculate_mse(prediction, gt)
        psnr_pre = calculate_psnr(prediction, gt)
        ssim_pre = calculate_ssim(prediction, gt)
        ergas_pre = calculate_ergas(prediction, gt)
        lpips_pre = calculate_lpips(prediction, gt)

        avg_mse_predicted = avg_mse_predicted + mse_pre
        avg_psnr_predicted = avg_psnr_predicted + psnr_pre
        avg_ssim_predicted = avg_ssim_predicted + ssim_pre
        avg_ergas_predicted = avg_ergas_predicted + ergas_pre
        avg_lpips_predicted = avg_lpips_predicted + lpips_pre

        # save imges
        img_name = os.path.split(im_path[0])[1]
        sr_folder = args_test.save_results_dir + '/sig' + str(args_test.sig) + '/SR/'
        save_img(prediction, sr_folder, img_name)

        result_imgs = [gt, lr_d, prediction]
        mses = [None, None, mse_pre]
        psnrs = [None, None, psnr_pre]
        ssims = [None, None, ssim_pre]
        ergass = [None, None, ergas_pre]
        lpipss = [None, None, lpips_pre]
        plot_img(result_imgs, mses, psnrs, ssims, ergass, lpipss, '{}/{}_plot.png'.format(sr_folder, img_name.split('.')[0]))

        count += 1

    plot_folder = args_test.save_results_dir + '/sig' + str(args_test.sig) + '/validation_epoch_end.txt'
    f = open(plot_folder, 'a')
    print_metrics = 'SR test: sr_mse: %0.3e, sr_psnr: %0.3e, sr_ssim：%0.5e, sr_ergas: %0.4e, sr_lpips: %0.5e' % (
        avg_mse_predicted / count, avg_psnr_predicted / count, avg_ssim_predicted / count, avg_ergas_predicted / count, avg_lpips_predicted / count)
    f.write(print_metrics + os.linesep)
    f.write('total_time:' + str(total_time))
    f.close()

    print("MSE_predicted=", avg_mse_predicted / count)
    print("PSNR_predicted=", avg_psnr_predicted / count)
    print("SSIM_predicted=", avg_ssim_predicted / count)
    print("ERGAS_predicted=", avg_ergas_predicted / count)
    print("LPIPS_predicted=", avg_lpips_predicted / count)
