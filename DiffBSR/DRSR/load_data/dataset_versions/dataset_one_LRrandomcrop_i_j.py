...
class DatasetFromFolder(data.Dataset):  # 读取一张HR，按patch返回HR，lr,退化模糊核b_kernel以及lr的两个正样本lr_i，lr_j
    ...
    def __getitem__(self, index):
        target = load_img(self.hr_image_filenames[index])  # 读取图片
        patch_gt = get_patch(target, self.patch_size)  # 随机裁剪patch, numpy:[h w 3]

        if self.data_augmentation:  # 旋转翻转数据扩充
            patch_gt = augment(patch_gt)

        patch_gt = np2Tensor(patch_gt).unsqueeze(0).unsqueeze(0).cuda()  # (h w 3)numpy image要先转成tensor[1, 1, 3, h, w](rgbrange = 255)
        input_d, bkernel = self.degrade(patch_gt, random=True)  # degradation

        input_d = input_d.squeeze(0).squeeze(0)
        patch_gt = patch_gt.squeeze(0).squeeze(0)

        if self.transform:  # 在LR中随机裁剪LR的size//2（可自定义） + 随机旋转, 返回两个数据增强的inputi，j
            input_i, input_j = self.transform(input_d)

        return patch_gt, input_d, input_i, input_j
