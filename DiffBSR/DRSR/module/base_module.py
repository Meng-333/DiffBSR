...
def save_img(img, img_path, img_name):
    save_dir = os.path.join(img_path)
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    save_fn = save_dir + img_name
    img = img.astype(np.uint8)
    io.imsave(save_fn, img)
...
def plot_img(imgs, mses, psnrs, ssims, ergas, lpips, save_fn, show_label=True, show=False):
    ...
    for i, (ax, img, mse, psnr, ssim, erga, lpip) in enumerate(zip(axes.flatten(), imgs, mses, psnrs, ssims, ergas, lpips)):
        ...
        #print('________img.shape', img.shape)   # [3, 256, 256]
        ...
        if show_label:
            ...
            if i == 0:
                ...
                ax.set_xlabel('SR image (MSE: %.3fdB)\n (PSNR: %.3fdB)\n (SSIM: %.5f)\n (ERGA: %.4f)\n (LPIP: %.5f)' % (mse, psnr, ssim, erga, lpip))
...
def calculate_ssim(img1, img2):
    return compare_ssim(img1, img2, win_size=3, multichannel=True)
...
def calculate_lpips(img1, img2, use_gpu=True):
    ...
    test_HR = torch.FloatTensor(img2) / 255.0
    test2 = torch.FloatTensor(img1) / 255.0
