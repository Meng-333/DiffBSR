...
def make_dataloaders(datasets, type, scale, config):
    ...
    for dataset_name in datasets:
        if type == "LRHR_paired":
            ...
            dataset = SingleImageDataset(
                #img_path="load/AIRS_test".format(dataset_name),
                img_path="load/WHU_test".format(dataset_name),
                #img_path="load/UCM_test/tenniscourt".format(dataset_name),
                rgb_range=config.data_module.args.rgb_range,
                cache="bin",
                mean=config.data_module.args.get("mean"),
                std=config.data_module.args.get("std"),
...
def test(args):
    ...
    #f = open('./logs/blindsrsnf_iso/test_log.txt', 'a')
    #f = open('./logs/blindsrsnf_aniso/test_log.txt', 'a')
    #f = open('./logs/blindsrsnf_aniso_UCM/test_log.txt', 'a')
    #f = open('./logs/ddpm_diffbsr_aniso/test_log.txt', 'a')
    #f = open('./logs/srdiffbsr_aniso/test_log.txt', 'a')
    #f = open('./logs/diffbsr_iso/test_log.txt', 'a')
    f = open('./logs/diffbsr_aniso/test_log.txt', 'a')
    #f = open('./logs/diffbsr_aniso_UCM/test_log.txt', 'a')
    ...
    for dataset_name, loader in dataloaders:
        ...
        if config.lit_model.args.blur_type == "iso_gaussian":
            ...
            print1 = "==== Dataset: {}, Scale Factor: x{:.2f}, Sigma: {:.2f}, noise: {:.2f}, eta {:.2f}, skip {:.2f}====".format(
                    dataset_name, scale, args.sigma, args.noise, args.eta, args.skip)
            f.write(print1 + os.linesep)
        ...
        if config.lit_model.args.blur_type == "aniso_gaussian":
            ...
            print( "==== Dataset: {}, Scale Factor: x{:.2f}, Lambda: [{:.2f}, {:.2f}], theta {:.2f}, noise {:.2f}, eta {:.2f}, skip {:.2f} ====".format(
                    dataset_name,
                    scale,
                    args.lambda1,
                    args.lambda2,
            ...
            print2 = "==== Dataset: {}, Scale Factor: x{:.2f}, Lambda: [{:.2f}, {:.2f}], theta {:.2f}, noise {:.2f}, eta {:.2f}, skip {:.2f} ====".format(
                    dataset_name,
                    scale,
                    args.lambda1,
                    args.lambda2,
        ...
        for batch in tqdm(loader, total=len(loader.dataset)):
            ...
            if "val_blpips" in rslt.keys():
                blpipss.append(rslt["val_blpips"].item())
            ...
            if "val_lpips" in rslt.keys():
                lpipss.append(rslt["val_lpips"].item())
        ...
        # If plot_img.png is also saved, the FID metric needs to be calculated separately in FID.py
        if "HR" in args.datatype:

            #hr_path = "load/AIRS_test".format(dataset_name)
            hr_path = "load/WHU_test".format(dataset_name)
            #hr_path = "load/UCM_test/tenniscourt".format(dataset_name)

            paths = [rslt_path, hr_path]
            fid_score = calc_fid(paths)  # If only sr.tif is saved, the results here can be used
            print("- FID : {:.5f}".format(fid_score))
            print16 = "- FID : {:.5f}".format(fid_score)
            f.write(print16 + os.linesep)
...
def getTestParser():
    ...
    parser.add_argument(
        #"--datasets", default="AIRS_test", type=str, help="dataset names"
        "--datasets", default="WHU_test", type=str, help="dataset names"
        #"--datasets", default="UCM_test", type=str, help="dataset names"
    )
    parser.add_argument(
        "--datatype",
        default="HR_only",
    ...
    parser.add_argument("--schedule", default="linear_cosine")
