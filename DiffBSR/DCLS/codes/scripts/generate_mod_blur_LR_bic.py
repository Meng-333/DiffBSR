...
try:
    ...
    # from utils.deg_utils import SRMDPreprocessing
    # from utils.dcls_utils import *
    # from utils.file_utils import set_random_seed
...
def generate_mod_LR_bic():
    ...
    sourcedir = "/data/load_160/"
    savedir = "/data/load_160_DLSR/"

    # load PCA matrix of enough kernel
    print("load PCA matrix")
    pca_matrix = torch.load(
        "/root/autodl-tmp/DCLS/pca_matrix/DCLS/pca_matrix.pth", map_location=lambda storage, loc: storage
    )
    ...
    filepaths = sorted([f for f in os.listdir(sourcedir) if f.endswith(".tif")])
    print(filepaths)
    ...
    for i in range(num_files):
        ...
        for sig in np.linspace(1.8, 3.2, 8):
            ...
            #image_kernel = tensor2img(kernel, min_max=(kernel.min().item(), kernel.max().item()))
