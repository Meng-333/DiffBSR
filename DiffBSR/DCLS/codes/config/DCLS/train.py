        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    # # BIC_img = LR.resize((256, 256), Image.BICUBIC)
                    # BIC_img = val_data["BIC"]
                    ...
                    # calculate MSE, PSNR, SSIM, ERGAS, LPIPS
                    ...
                    bic_mse += util.calculate_mse(bic_img, gt_img)
                    bic_psnr += util.calculate_psnr(bic_img, gt_img)
                    bic_ssim += util.calculate_ssim(bic_img, gt_img)
                    bic_ergas += util.calculate_ergas(bic_img, gt_img)
                    bic_lpips += util.calculate_lpips(bic_img, gt_img)
                    avg_mse += util.calculate_mse(sr_img, gt_img)
                    avg_psnr += util.calculate_psnr(sr_img, gt_img)
                    avg_ssim += util.calculate_ssim(sr_img, gt_img)
                    avg_ergas += util.calculate_ergas(sr_img, gt_img)
                ...
                if current_step == 1000:
                    logger.info("# Validation # BIC_MSE: %0.5e, BIC_PSNR: %0.5e, BIC_SSIM: %0.5e, BIC_ERGAS: %0.5e, BIC_LPIPS: %0.5e|"
                                " Iter: %d" % (bic_mse, bic_psnr, bic_ssim, bic_ergas, bic_lpips, current_step))

                logger.info("# Validation # SR_MSE: %0.5e, SR_PSNR: %0.5e, SR_SSIM: %0.5e, SR_ERGAS: %0.5e, SR_LPIPS: %0.5e|"
                                " Iter: %d" % (avg_mse, avg_psnr, avg_ssim, avg_ergas, avg_lpips, current_step))
                ...
                if current_step == 1000:
                    logger_val.info(
                        "<epoch: %d, iter: %d, bic_mse: %0.5e, bic_psnr: %0.5e, bic_ssim: %0.5e, bic_ergas: %0.5e, bic_lpips: %0.5e" % (
                            epoch, current_step, bic_mse, bic_psnr, bic_ssim, bic_ergas, bic_lpips))

                logger_val.info(
                    "<epoch: %d, iter: %d, sr_mse: %0.5e, sr_psnr: %0.5e, sr_ssim: %0.5e, sr_ergas: %0.5e, sr_lpips: %0.5e" % (
                        epoch, current_step, avg_mse, avg_psnr, avg_ssim, avg_ergas, avg_lpips))
